...
public HashMap<String, Integer> getKidsCountByCategory(int period){
	...
	for(String categ : categories) {
		int categoryKids = 0;
		List<Course> courses = courseRepo.getCategoryCourses(categ);
		for(Course c : courses) {
			if(c.getStartDateTime().after(d) & (c.getName()).indexOf(categ) == 0) {
				categoryKids += c.getKidsIDs().size()+1;
			}
		}
		kidsCountByCategory.put(categ, categoryKids);
	}
//	for(String catId : getAllCategoriesIds()) {
//		int categoryKids = 0;
//		List<Course> courses = courseRepo.getCategoryCourses(catId);
//		for(Course c : courses) {
